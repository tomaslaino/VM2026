...
npm run sync:summaries # skriv matchartiklarna + matchanalyserna automatiskt → data/news_summaries.json + data/match_analysis.json (Gemini skriver svensk förhandsartikel och en kort analys med prognos ur färska källor; kräver GEMINI_API_KEY). --dry-run testar utan nyckel
...
| `assets/matchinfo.js` | Matchmodal med detaljer/statistik + inför-snack ("Inför"-fliken) för ospelade matcher (data från `VMApp.matchPreview` i app.js; visar även en kort redaktionell analys med prognos ur `data/match_analysis.json` via `pvAnalysisHtml`, direkt under oddsbaren) samt fliken "Senaste nytt": automatgenererad svensk förhandsartikel per match ur `data/news_summaries.json` (rubrik/ingress/fet/kursiv + upphöjda `[[n]]`-källhänvisningar, renderas av `miInlineNews`), annars rubriklista ur `data/team_news.json`. Överst i båda fallen "Avbräck & frågetecken" ur spelarstatusen |
...
| `server/scripts/syncNewsSummaries.js` | Bygger matchartiklarna automatiskt. Läser kommande slutspelsmatcher ur `results.json` (fixtures, båda lag klara + avspark inom 6 dygn), samlar färska referenser (ländernas egna medier för båda lagen + en matchspecifik lokal sökning som nämner motståndaren + internationell förhandssökning + en sökning om spelavgörande förhållanden (höjd/värme/plan) + avbräck ur spelarstatusen; dedupas, åldersfiltreras, rankas – lokala/konkreta upp, rena listningar/logistik ned, översätts till svenska med gratis gtx). **Läser dessutom artiklarnas brödtext**: löser ut Google Nyheters omdirigeringslänkar till riktiga URL:er (batchexecute), hämtar sidan och extraherar ren prosa (bäst-möjligt, faller tillbaka på rubriken). Låter sedan Gemini skriva en tes-driven införartikel `{headline, lead, paragraphs}` med `[[n]]`-citat, `**fet**`/`*kursiv*` (prompten följer införartikel-mallen: MATCH-block + faktor-taxonomi + 2–4 viktigaste faktorer med motivering). **Trappa mot avspark**: regenererar glesare långt bort (var 24:e h) och tätare nära (ned till var 45:e min < 6 h kvar), bara när referenserna ändrats (`refsHash`). Slår primärmodellen i kvottaket (429) faller den över till `GEMINI_FALLBACK_MODEL` (gemini-2.0-flash). Körs var 30:e min av `sync-news-summaries.yml`. Kräver `GEMINI_API_KEY`; `--dry-run`/`--force`/`--match k:NN` för test. **Skriver även `data/match_analysis.json`**: ur samma referenser gör ett andra Gemini-anrop per match en kort bedömning med prognos (`{verdict, prediction, predictionNote, paragraphs}`, utan `[[n]]`-citat). Artikel och analys grindas oberoende (var sin `refsHash`/trappa) så den ena inte skrivs om i onödan för att den andra är due |
| `data/match_analysis.json` | Redaktionell matchanalys med prognos per kommande match (`k:NN`): `verdict` (rubrik), `prediction` (troligt resultat, t.ex. "2–1 Brasilien"), `predictionNote` (kort brasklapp) + `paragraphs` (kort motivering; stöder `**fet**`/`*kursiv*`, inga källhänvisningar). **Skrivs automatiskt** av `syncNewsSummaries.js` (Gemini, ur samma färska källor som artiklarna) – auto-poster får `generated: true` + `refsHash`; poster med `manual: true` bevaras (kan handskrivas). Visas överst i Inför-fliken (efter oddsbaren) via `pvAnalysisHtml` i `matchinfo.js`; saknas matchen visas bara marknadssiffrorna |
